    ...
    private final class AsyncMessageProcessingConsumer implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                LOGGER.debug("Consumer raised exception, processing can restart if the connection factory supports it", t);
    ...
    private boolean doReceiveAndExecute(BlockingQueueConsumer consumer) throws Throwable { //NOSONAR
        for (int i = 0; i < this.txSize; i++) {
            ...
            try {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("process message fail the reason is : " + e.getMessage() + "the message is " + message);
                }
                break;
            }
        }
